...
class Backend():
    ...
    def update_model_run_record(run_id, status=None, target_host_port='localhost:80', update_inference_output=False, model_run_src=''):
        ...
        response = requests.put(
            f"http://{target_host_port}/model", json=request_data)
    ...
    def database_new_model_run(self, model_type, playlist_selections, genre_selections, status, ts_created, ts_complete, time_total, time_training, time_inference):
        inference_output = []
        try:
            new_model_run = Backend.ModelRun(model_type=model_type, playlist_selections=playlist_selections, genre_selections=genre_selections, inference_output=inference_output, status=status,
        ...
        if method == "POST":  # POST
            ...
            if status_update != '':
                ...
                if not update_success:
                    return (flask.jsonify({
                        'success': False,
                        'message': 'Server error (failed to update new model run record status in database).'
                    }), 500)
            if inference_output_update != '' and inference_output_update != None:
                ...
                if not update_success:
                    return (flask.jsonify({
                        'success': False,
                        'message': 'Server error (failed to update new model run record inference output in database).'
                    }), 500)
            if ts_profile_update != '' and ts_profile_update != None:
                ...
                if not update_success:
                    return (flask.jsonify({
                        'success': False,
                        'message': 'Server error (failed to update new model run record time profile in database).'
                    }), 500)
            # return success
            return flask.jsonify({
                'success': True,
